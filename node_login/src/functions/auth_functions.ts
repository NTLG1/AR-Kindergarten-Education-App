        ...
        connection.query(query, user, (err: MysqlError | null, result: OkPacket | null) => {
            ...
            if (result != null) {
                if (result.affectedRows == 1) {
                    ...
                    reject('Unknown error 1 saving user to database');
                    return;
                }
            } else {
                reject('Unknown error 2 saving user to database');
                return;
            }
        });
    })));
...
const createAndAddJwtToRes = function createAndAddJwtToRes(email: string, username: string, res: Response) {
    const json = jwt.sign({email, username}, process.env.JWT_SECRET, {expiresIn: 86400 * 30}); // 1 month
    res.cookie('jwt', json, { httpOnly: true, maxAge: 86400 * 30 * 1000});
}
        ...
        connection.query(query, passwordResetObject, (err: MysqlError | null, result: OkPacket | null) => {
            ...
            if (result != null) {
                if (result.affectedRows == 1 || result.affectedRows == 2) {
                    ...
                    reject('Unknown error 1 saving password reset to database');
                    return;
                }
            } else {
                reject('Unknown error 2 saving password reset to database');
            }
        })
    }))
}
    ...
    return new Promise<string>(((resolve, reject) => {
        ...
        connection.query(query, (err: MysqlError | null, result: any) => {
            ...
            if (result != null && result.length == 1) {
                ...
                reject('Unknown error 1 getting reset token from database');
                return;
            }
        });
    }));
